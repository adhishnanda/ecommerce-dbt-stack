...
Kaggle CSVs
    │
    │  Python ingestion script (google-cloud-bigquery)
    ▼
BigQuery: raw_ecommerce          9 source tables, schema autodetected
    │
    │  dbt run
    ▼
BigQuery: staging                7 views  — rename columns, cast types, 1:1 with source
    │
    ▼
BigQuery: intermediate           3 views  — joins, business logic, delivery metrics
    │
    ▼
BigQuery: marts                  3 tables — Kimball star schema, optimised for BI
    │
    ▼
Looker Studio Dashboard          Live, shareable, geo-enabled
```
